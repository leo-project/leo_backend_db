...
profile_output() ->
    ...
    eprof:log("leo_backend_db.procs.profile"),
    eprof:analyze(procs),
    eprof:log("leo_backend_db.total.profile"),
    eprof:analyze(total).


-spec consider_profiling() -> profiling | not_profiling | {error, any()}.
consider_profiling() ->
    case application:get_env(leo_backend_db, profile) of
        {ok, true} ->
            {ok, _Pid} = eprof:start(),
